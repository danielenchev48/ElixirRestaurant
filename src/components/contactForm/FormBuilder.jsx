...
const FormBuilder = ({configurations}) => {
    ...
    const validateForm = () => {
        ...
        if (!name || name.length < 2 || name.length > 100) {
          validationErrors.name = "Name is required and should be between 2 and 100 characters";
        ...
        if (!subject || subject.length < 10 || subject.length > 200 || !/^[A-Z]/.test(subject)) {
          validationErrors.subject = "Subject is required and should be between 10 and 200 characters, starting with a capital letter";
          setErrorMsg(validationErrors.subject)
          console.log(validationErrors.subject);
        }
    ...
    const handleSubmit = (event) => {
        ...
        if(validateForm()){
            axios.post('http://localhost:3000/api/v1/contacts/sendMessage', formData)
            .then((res) => {
            ...
            {configurations.map((inputData, index) => inputData.label ?
             (<FormGroup controlId={inputData.name}>
                ...
                <FormControl 
                    ...
                    minLength={inputData.minLength ? inputData.minLength : undefined}
                    maxLength={inputData.maxLength? inputData.maxLength : undefined}
